...
export class ExportService {
  ...
  async exportBillsReportPdf(query: BillReportQuery, branding: Record<string, string>): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.y = boxY + 56;
      const widths = [28, 55, 95, 110, 68, 68, 68, 68, 72]; const headers = ['No', 'Periode', 'Jenis', 'Keluarga', 'Tagihan', 'Dibayar', 'Sisa', 'Status', 'Jatuh Tempo']; const totalWidth = widths.reduce((a, b) => a + b, 0);
      const drawRow = (values: string[], header = false) => { const y = doc.y; let x = 36; if (header) doc.rect(36, y - 3, totalWidth, 19).fillAndStroke('#f3e8d0', '#171717'); doc.font(header ? 'Helvetica-Bold' : 'Helvetica').fontSize(7.5).fillColor('#171717'); values.forEach((value, i) => { doc.text(value, x + 3, y + (header ? 2 : 0), { width: widths[i] - 6, height: 13, ellipsis: true }); x += widths[i]; }); doc.y = y + 19; doc.moveTo(36, doc.y - 2).lineTo(36 + totalWidth, doc.y - 2).lineWidth(header ? 1.2 : 0.4).strokeColor('#b8aa90').stroke(); };
      drawRow(headers, true); report.rows.forEach((row, i) => { if (doc.y > 520) { doc.addPage(); drawHeader(); drawRow(headers, true); } drawRow([String(i + 1), row.period, row.billType, row.family, rupiah(row.amount), rupiah(row.paid), rupiah(row.outstanding), statusLabel(row.status), dateId(row.dueDate)]); }); if (!report.rows.length) doc.font('Helvetica').fontSize(11).text('Tidak ada data untuk filter yang dipilih.', 36, doc.y + 15);
      const footer = (pageNumber: number) => { const y = 560; doc.moveTo(36, y).lineTo(36 + pageWidth, y).strokeColor('#b8aa90').stroke(); doc.font('Helvetica').fontSize(8).fillColor('#6b7280').text('Dokumen dihasilkan oleh WargaNet • Laporan resmi administrasi RT/RW', 36, y + 8); doc.text(`Halaman ${pageNumber}`, 36 + pageWidth - 70, y + 8, { width: 70, align: 'right' }); }; footer(1); doc.end();
    });
  }
