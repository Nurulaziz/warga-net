...
export function HashtagPostsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetchPostsByHashtag(tag, { page, limit: pageSize });
      setPosts(res.data);
      setMeta(res.meta);
    } catch {
      setPosts([]);
    ...
    <div className="mx-auto max-w-2xl space-y-4 p-4">
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">#{tag}</h1>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState title="Belum ada posting dengan tag ini" />
      ) : (
        <div className="space-y-3">
          {posts.map((post) => (
